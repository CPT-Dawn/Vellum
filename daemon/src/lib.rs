...
pub struct AwwwServerConfig {
    ...
    pub quiet: bool,
    /// If true, skip loading cached wallpapers on startup.
    pub no_cache: bool,
    /// Wayland layer-shell layer used for wallpaper surfaces.
    pub layer: Layer,
    /// Namespace suffix used to isolate socket and layer-shell instances.
...
impl Daemon {
    fn new(
        ...
        Self {
            ...
            layer_shell,
            layer: config.layer,
            pixel_format: config.format.unwrap_or(PixelFormat::Argb),
...
/// Returns whether a daemon instance responds on the given namespace socket.
